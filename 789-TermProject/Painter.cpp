...
SoSeparator* Painter::getSphereSep(Mesh* mesh, float deltaX, float deltaY, float scale)
{
	...
	for (int i = 0; i < (int) mesh->samples.size(); i++)
	{
		...
		float * temp = ARAP::toFloat3(mesh->verts[ mesh->samples[i] ]->coord);
		...
		if (i == 0)
			ma->diffuseColor.setValue(SbColor(1.0f, 0.0f, 0.0f));
		else if (i == mesh->samples.size()-1)
			ma->diffuseColor.setValue(SbColor(0.0f, 1.0f, 0.0f));
		//else if (i == 2)
		...
		//	ma->diffuseColor.setValue(SbColor(0.7f, 0.7f, 0.0f));
			...
			ma->diffuseColor.setValue(SbColor(0.0f, 0.0f, 0.5f));

		sphere1Sep->addChild(ma);
		
		//shape
		SoSphere* sph1 = new SoSphere();
		if (i == 0 || i == mesh->samples.size() - 1)
			sph1->radius = radius * 3;
		else
			sph1->radius = radius *3;
...
SoSeparator* Painter::getShapeSep(Mesh* mesh)
{
	...
	mat->diffuseColor.setValue(0.7f, 0.3f, 1.0f); //paint all vertices with this color
	...
	if (youWantToPaintEachVertexDifferently)
		for (int i = 0; i < (int) mesh->verts.size(); i++) //i = 0 obj->color above overwritten here
			mat->diffuseColor.set1Value(i, ARAP::toFloat3(mesh->verts[i]->color)); //vert color according to its x-y-z coord (for mesh1) and to the transferred color (for mesh2)
	...
	for (size_t c = 0; c < mesh->verts.size(); c++)
		coords->point.set1Value(c, mesh->verts[c]->coord[0], mesh->verts[c]->coord[1], mesh->verts[c]->coord[2]);
	SoIndexedFaceSet* faceSet = new SoIndexedFaceSet();
	for (size_t c = 0; c < mesh->tris.size(); c++)
	{
	...
	if (drawThickEdges) //draw thick edges (may be useful in geodesic path drawing)
	{
		...
		SoDrawStyle* sty = new SoDrawStyle;	sty->lineWidth = 7.0f;	thickEdgeSep->addChild(sty);
		...
		{
		  float * temp1 = ARAP::toFloat3(mesh->verts[mesh->sedges[se]->v1i]->coord),
		        * temp2 = ARAP::toFloat3(mesh->verts[mesh->sedges[se]->v2i]->coord);
